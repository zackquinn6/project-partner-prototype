-- Clean up any previous attempt and consolidate Jig Saw Blade items into variants
-- First, delete any existing "Jig Saw Blade" tool and its related data
DELETE FROM public.variation_instances WHERE core_item_id IN (SELECT id FROM public.tools WHERE item = 'Jig Saw Blade');
DELETE FROM public.variation_attribute_values WHERE core_item_id IN (SELECT id FROM public.tools WHERE item = 'Jig Saw Blade');
