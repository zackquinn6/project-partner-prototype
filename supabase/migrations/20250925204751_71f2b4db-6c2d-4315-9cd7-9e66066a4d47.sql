-- Clean up completely and consolidate Jig Saw Blade items into variants
-- First, clean up any existing data
DELETE FROM public.variation_instances WHERE core_item_id IN (SELECT id FROM public.tools WHERE item = 'Jig Saw Blade');
DELETE FROM public.variation_attribute_values WHERE attribute_id IN (SELECT id FROM public.variation_attributes WHERE name = 'type');
DELETE FROM public.variation_attributes WHERE name = 'type';
DELETE FROM public.tools WHERE item = 'Jig Saw Blade';

-- Now create the main "Jig Saw Blade" tool
...
-- Get the ID of the newly created tool and create variations
WITH new_tool AS (
  SELECT id FROM public.tools WHERE item = 'Jig Saw Blade'
),
-- Create the "Type" attribute
new_attribute AS (
  INSERT INTO public.variation_attributes (name, display_name, attribute_type)
  VALUES ('type', 'Type', 'select')
  RETURNING id as attribute_id
),
